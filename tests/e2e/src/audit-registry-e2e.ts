/**
 * Audit Event Registry E2E — verify all 63 event types insert + query.
 *
...
async function main(): Promise<void> {
  console.log('=== Audit Event Registry E2E: 63 event types ===\n');
  ...
  assert(inserted === 63, `should insert 63 events, got ${inserted}`);
  console.log(`✓ Inserted ${inserted}/63 event types`);

  // Verify count via store.query
  const allEvents = await store.query({});
  assert(allEvents.length === 63, `total events should be 63, got ${allEvents.length}`);
  ...
  console.log('✓ All 63 event types individually queryable');
  ...
  // Verify count is 63 (not 39 from old registry)
  assert(EVENT_TYPES.length === 63, `AUDIT_EVENT_TYPES should have 63 entries, got ${EVENT_TYPES.length}`);
