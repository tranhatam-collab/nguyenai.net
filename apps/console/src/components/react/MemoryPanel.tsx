...
export default function MemoryPanel() {
  ...
  const load = useCallback(async (type?: string) => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : 'Tải bộ nhớ thất bại.');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  const handleWrite = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : 'Ghi bộ nhớ thất bại.');
    }
  ...
  const handleDelete = useCallback(async (key: string) => {
    try {
      ...
      setError(err instanceof ApiError ? err.message : 'Xóa bộ nhớ thất bại.');
    }
  }, [filter, load]);
  ...
  return (
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center gap-3">
        <label className="text-xs text-slate-400" htmlFor="memory-filter">Lọc:</label>
        <select
          id="memory-filter"
        ...
        <button
          ...
          aria-label={showForm ? "Hủy" : "Ghi bộ nhớ"}
        >
      ...
      {showForm && (
        <div className="console-card mb-4">
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
            <div>
              <label className="console-label">Loại</label>
              <select
            ...
            <div>
              <label className="console-label" htmlFor="memory-key">Khóa</label>
              <input
                id="memory-key"
                className="console-input"
                value={newMemory.key}
                onChange={(e) => setNewMemory((m) => ({ ...m, key: e.target.value }))}
                placeholder="vd: ngon_ngu_uu_tien"
              />
            </div>
            <div>
              <label className="console-label" htmlFor="memory-value">Giá trị</label>
              <input
                id="memory-value"
                className="console-input"
                value={newMemory.value}
          ...
          <button className="console-btn console-btn-primary mt-3 text-xs" onClick={handleWrite} aria-label="Lưu bộ nhớ">
            Lưu
          </button>
        </div>
      )}
      ...
      {!loading && memories.length === 0 && (
        <div className="console-card">
          <p className="text-sm text-slate-400">Chưa có bộ nhớ nào.</p>
        </div>
      )}
              ...
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    {new Date(m.updated_at).toLocaleString()} · {m.tags.join(', ') || 'không có nhãn'}
                  </p>
                </div>
                <button
                  className="text-xs text-red-400 hover:text-red-300"
                  onClick={() => handleDelete(m.key)}
                  aria-label={`Xóa bộ nhớ ${m.key}`}
                >
                  Xóa
                </button>
              </div>
