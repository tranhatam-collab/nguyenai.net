...
const vi: Record<RouteKey, PageContent> = {
  ...
  terms: {
    ...
    sections: [
      ...
      { title: '10. Liên hệ', body: 'Email: legal@nguyenai.net. Địa chỉ VN: Kasan JSC, ACB HCM Branch, tài khoản 3051378.' }
    ]
  },
  privacy: {
    ...
    sections: [
      ...
      { title: '3. Mã hóa', body: 'Data Vault mã hóa at-rest và in-transit. Memory mã hóa at-rest. Mật khẩu hash PBKDF2 600K iterations. Session token hash SHA-256. API keys hash SHA-256. MFA TOTP secret mã hóa.', items: ['At-rest: AES-256 cho Data Vault', 'In-transit: TLS 1.3', 'Password: PBKDF2 600K iterations + SHA-256', 'Session: SHA-256 hash + expiry', 'API keys: SHA-256 hash', 'MFA: TOTP secret encrypted'] },
      { title: '4. Quyền của bạn', body: 'Bạn có quyền: truy cập, chỉnh sửa, xóa, export, hạn chế xử lý, phản đối xử lý. Export đầy đủ memory, vault, audit log bất cứ lúc nào. Xóa tài khoản = xóa toàn bộ dữ liệu trong 30 ngày.', items: ['Truy cập: xem toàn bộ dữ liệu của bạn', 'Chỉnh sửa: sửa thông tin tài khoản', 'Xóa: xóa tài khoản + toàn bộ dữ liệu', 'Export: memory + vault + audit log', 'Hạn chế: tạm dừng xử lý', 'Phản đối: từ chối xử lý cụ thể'] },
      { title: '5. PDPD 91/2025/QH15', body: 'Tuân thủ Luật Bảo vệ dữ liệu cá nhân Việt Nam 91/2025/QH15. Kasan JSC (MST 0315521422) là đại diện thương mại tại Việt Nam, chịu trách nhiệm PDPD compliance cho khách hàng Việt Nam. VIET CAN NEW CORP chịu trách nhiệm quốc tế.', items: ['PDPD 91/2025/QH15 — Luật Bảo vệ dữ liệu cá nhân VN', 'Kasan JSC — PDPD compliance cho khách VN', 'VIET CAN NEW CORP — compliance quốc tế', 'Data Processing Agreement có sẵn cho enterprise'] },
      { title: '6. Chia sẻ dữ liệu', body: 'Không bán dữ liệu cá nhân. Chia sẻ với: AI model providers (anonymized), payment gateways (Stripe, VNPay — giao dịch only), hosting (Cloudflare — infrastructure only). Enterprise có DPA riêng.' },
      { title: '7. Lưu trữ và xóa', body: 'Dữ liệu lưu trong thời gian tài khoản active. Xóa tài khoản = xóa toàn bộ trong 30 ngày. Audit log lưu 7 năm (yêu cầu pháp lý). Backup lưu 90 ngày rồi xóa.' },
      { title: '8. Cookie', body: 'Session cookie (essential). Không tracking cookie. Không third-party ads. Không Facebook Pixel. Không Google Analytics tracking cá nhân.' },
      { title: '9. Trẻ em', body: 'Không thu thập dữ liệu trẻ em dưới 13 tuổi. Family plan: phụ huynh quản lý, trẻ em truy cập giới hạn theo quyền thế hệ.' },
      { title: '10. Liên hệ', body: 'Email: privacy@nguyenai.net. PDPD officer: Kasan JSC, MST 0315521422.' }
    ]
  }
};
...
const en: Record<RouteKey, PageContent> = {
  ...
  terms: {
    key: 'terms', title: 'Terms of Service — Nguyen AI Computer', description: 'Nguyen AI Computer terms of service: acceptance, plans, payment, refunds, liability limits, legal entities.', eyebrow: 'Terms', heroTitle: 'Nguyen AI Computer Terms of Service.', heroText: 'By using Nguyen AI Computer, you agree to the terms below. Please read carefully before initializing a machine or making a payment.',
    sections: [
      { title: '1. Acceptance of terms', body: 'By creating an account, initializing an AI Computer, or using any Nguyen AI service, you agree to be bound by these terms. If you do not agree, do not use the service.' },
      { title: '2. Definitions', body: 'Nguyen AI Computer is a personal AI Computer system on the cloud. A Model is a machine tier. A Functional Product is a specialized tool bundle. A Super App is a specialized application. An Agent is a specialized AI agent.', items: ['Nguyen AI — brand', 'Nguyen AI Computer — product', 'Model — machine tier', 'Functional Product — tool bundle', 'Super App — specialized application', 'Agent — specialized AI agent'] },
      { title: '3. Plans and pricing', body: 'Current prices are pilot hypotheses, subject to validation of AI, storage, support and legal costs. Each plan has clear capacity, limits and status (Available, Beta, Planned, Enterprise only). No non-existent feature is announced as complete.' },
      { title: '4. Payment', body: 'Payment is monthly or annual. VND via VNPay or bank transfer to Kasan JSC (commercial representative). USD via Stripe or wire transfer. Vietnam VAT invoices issued by Kasan JSC for Vietnam customers.' },
      { title: '5. Refunds', body: '100% refund within the first 7 days if you have used fewer than 10 commands. After 7 days, prorated refund minus AI costs consumed. Academy Pass and Certification Fee are non-refundable once use begins.' },
      { title: '6. Limitation of liability', body: 'Nguyen AI Computer provides analysis tools, not licensed legal, financial or medical advice. Agent and AI output is assistance, not a replacement for professionals. Evidence is a verification tool, not confirmation of bloodline or royal descent.' },
      { title: '7. Privacy', body: 'Personal data is protected under PDPD 91/2025/QH15 (Vietnam) and our separate privacy policy. Living-person data is private by default. Family trees are private by default. See the Privacy page for details.' },
      { title: '8. Legal entities', body: 'VIET CAN NEW CORP (US) bears full legal responsibility for founding, operating the system and owns all IP. Kasan JSC (Tax ID 0315521422) is a commercial representative in Vietnam — issues VAT, complies with PDPD, receives VND payments. Kasan JSC does not own IP and does not bear primary legal liability.', items: ['VIET CAN NEW CORP (US) — primary legal entity, formation in progress', 'Kasan JSC (VN, Tax ID 0315521422) — commercial representative', 'IP owned by VIET CAN NEW CORP', 'SAFE / Convertible Note issued by VIET CAN NEW CORP'] },
      { title: '9. Changes to terms', body: 'We may update these terms. 30 days notice before changes take effect. Continued use after 30 days constitutes acceptance of the new terms.' },
      { title: '10. Contact', body: 'Email: legal@nguyenai.net. Vietnam address: Kasan JSC, ACB HCM Branch, account 3051378.' }
    ]
  },
  privacy: {
    key: 'privacy', title: 'Privacy Policy — Nguyen AI Computer', description: 'Nguyen AI Computer privacy policy: personal data, PDPD 91/2025/QH15, encryption, user rights, living-person data.', eyebrow: 'Privacy', heroTitle: 'Nguyen AI Computer Privacy Policy.', heroText: 'We protect personal data under PDPD 91/2025/QH15 (Vietnam) and international best practice. Each user has a private instance, data is not shared.',
    sections: [
      { title: '1. Data we collect', body: 'Account data: email, name, password (hash). Operational data: commands, memory, vault, audit log. Payment data: invoices, transactions (no card storage). Optional data: founder profile, genealogy, oral history.', items: ['Account: email, name, password hash (PBKDF2 600K iterations)', 'Operational: commands, memory, vault, audit log', 'Payment: invoices, transactions (no card number stored)', 'Optional: profiles, genealogy, oral history'] },
      { title: '2. Default principles', body: 'Living-person data is private by default. Family trees are private by default. Family documents are private until owner publishes. Founder profiles are public only after owner approval. No data is public by default.', items: ['Living-person data: private by default', 'Family trees: private by default', 'Family documents: private until publish', 'Founder profiles: public only after approval'] },
      { title: '3. Encryption', body: 'Data Vault is encrypted at-rest and in-transit. Memory is encrypted at-rest. Passwords are hashed with PBKDF2 600K iterations. Session tokens are SHA-256 hashed. API keys are SHA-256 hashed. MFA TOTP secrets are encrypted.', items: ['At-rest: AES-256 for Data Vault', 'In-transit: TLS 1.3', 'Password: PBKDF2 600K iterations + SHA-256', 'Session: SHA-256 hash + expiry', 'API keys: SHA-256 hash', 'MFA: TOTP secret encrypted'] },
      { title: '4. Your rights', body: 'You have the right to: access, rectify, erase, export, restrict processing, object to processing. Full export of memory, vault and audit log at any time. Account deletion = full data deletion within 30 days.', items: ['Access: view all your data', 'Rectify: edit account information', 'Erase: delete account + all data', 'Export: memory + vault + audit log', 'Restrict: pause processing', 'Object: refuse specific processing'] },
      { title: '5. PDPD 91/2025/QH15', body: 'Compliance with Vietnam Personal Data Protection Law 91/2025/QH15. Kasan JSC (Tax ID 0315521422) is the commercial representative in Vietnam, responsible for PDPD compliance for Vietnam customers. VIET CAN NEW CORP handles international compliance.', items: ['PDPD 91/2025/QH15 — Vietnam Personal Data Protection Law', 'Kasan JSC — PDPD compliance for VN customers', 'VIET CAN NEW CORP — international compliance', 'Data Processing Agreement available for enterprise'] },
      { title: '6. Data sharing', body: 'We do not sell personal data. We share with: AI model providers (anonymized), payment gateways (Stripe, VNPay — transaction only), hosting (Cloudflare — infrastructure only). Enterprise has a separate DPA.' },
      { title: '7. Retention and deletion', body: 'Data is retained while the account is active. Account deletion = full deletion within 30 days. Audit logs are retained for 7 years (legal requirement). Backups are retained for 90 days then deleted.' },
      { title: '8. Cookies', body: 'Session cookie (essential). No tracking cookies. No third-party ads. No Facebook Pixel. No personal Google Analytics tracking.' },
      { title: '9. Children', body: 'We do not collect data from children under 13. Family plan: parents manage, children have limited access based on generation rights.' },
      { title: '10. Contact', body: 'Email: privacy@nguyenai.net. PDPD officer: Kasan JSC, Tax ID 0315521422.' }
    ]
  }
};
