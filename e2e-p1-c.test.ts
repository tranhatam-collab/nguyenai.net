...
import { WorkflowEngine, Workflow, WorkflowStep, StepStatus } from './packages/@nai/aqueduct/src/index.ts';
import { fetchPage, createCrawlSession, crawlStep } from './packages/@nai/scout/src/index.ts';
import { createCrew, assignAgent, executeCrew } from './packages/@nai/crew/src/index.ts';
import { createPipeline, addStage, executePipeline } from './packages/@nai/pipeline/src/index.ts';
...
async function runE2ETests() {
  ...
  try {
    console.log('Test 5: Pipeline Execution...');
  ...
  try {
    console.log('Test 7: Workflow without approval requirement...');
    const workflowEngine = new WorkflowEngine();
    const workflow: Workflow = {
