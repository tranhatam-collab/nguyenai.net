...
async function testPlanCancel() {
  ...
  const ent = await resolveEntitlements('u1', 't1', 'start');
  assert(ent.machine.plan === 'free', 'entitlement shows free plan');
}
...
async function testPlanDowngrade() {
  ...
  // Verify entitlements updated
  const ent = await resolveEntitlements('u1', 't1', 'nguyen-personal');
  assert(ent.machine.plan === 'personal', 'entitlement shows personal plan');
}
...
main();
  ...
  // Verify entitlements updated (resolve with new plan)
  const ent = await resolveEntitlements('u1', 't1', 'nguyen-personal');
  assert(ent.machine.plan === 'personal', 'entitlement shows personal plan');
}
