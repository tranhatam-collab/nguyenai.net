...
export async function resolveEntitlements(
  ...
  const result: ResolvedEntitlements = {
    machine: {
      ...
      instance_count: Number(planEnt['machine.instance.count'] ?? 1),
      model_tier: String(planEnt['machine.model.tier'] ?? 'free'),
      command_quota: Number(planEnt['machine.command.quota'] ?? 0),
      tokens_quota: Number(planEnt['machine.tokens.quota'] ?? 0),
      agents_enabled: (planEnt['machine.agents.enabled'] as string[]) ?? [],
      super_apps_enabled: (planEnt['machine.super_apps.enabled'] as string[]) ?? [],
      approval_required: String(planEnt['machine.approval.required'] ?? 'all'),
    },
    academy: {
      pass: Boolean(planEnt['academy.pass'] ?? false),
      tracks_enabled: (planEnt['academy.tracks.enabled'] as string[]) ?? [],
      lessons_limit: Number(planEnt['academy.lessons.limit'] ?? 0),
      cert_attempts: Number(planEnt['academy.cert.attempts'] ?? 0),
      cert_discount: Number(planEnt['academy.cert.discount'] ?? 0),
    },
    custom: {},
  };
